...
////////////====[MUTE]====////////////
if(strpos($message, "/mutear ") === 0 and $userId == $config['adminID']){
...
////////////====[BAN]====////////////
if(strpos($message, "/banear ") === 0 and $userId == $config['adminID']){
...
if(strpos($message, "/admin") === 0 and $userId == $config['adminID']){
  bot('sendmessage',[
      'chat_id'=>$chat_id,
      'text'=>"<b>Hola Admin! Tus Comandos:

/mutear - Mutea por tiempo especifico
    Ex:-    <code>/muteear userID 10m - Mute for 10 min
       /mutear userID 10d - Mute for 10 dias</code>

/unmute - Unmute r
    Ex:-    <code>/unmute userID - Unmute the User</code>

/ban - Permanently Ban a user
    Ex:-    <code>/banear userID - Ban User</code>    
    
/unban - Unban a Banned user
    Ex:-    <code>/unban userID - Unban the User</code>   
...
if(strpos($message, "/mutelist") === 0 and $userId == $config['adminID']){
  $mutelist = fetchMutelist();
   ...
   foreach ($mutelist as $result){
     ...
     $banuser .= $result." Muteado el ".date("F j, Y, g:i a",$timer['mute_timer'])."\n";
...
if(strpos($message, "/botstatss") === 0 and $userId == $config['adminID']){
...
- <ins>Usuarios :</ins> ".$totalrows."
- <ins>Total Ban:</ins> ".totalBanned()."
- <ins>Total Mute:</ins> ".totalMuted()."
